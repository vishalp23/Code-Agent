 ...
 */
async function queryLLMForErrorResolution(prompt) {
    const response = await (0, node_fetch_1.default)('https://api.openai.com/v1/chat/completions', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
 ...
 */
async function developProject(query, projectFolder) {
    const prompt = `
 ...
 */
async function runFullProjectWorkflow(query) {
 ...
 */
async function runDevelopmentPhase(query) {
    const workspaceDir = await ensureWorkspace();
    if (!workspaceDir) {
 ...
 */
async function provideOutline(userQuery) {
    const prompt = `
 ...
 */
function postOutlineWithOptions(panel, outlineText, userQuery) {
...
function activate(context) {
    ...
    ensureWorkspace().then(ws => {
        ...
        const setupProjectDisposable = vscode.commands.registerCommand('codeagent.setupProject', async (query) => {
        ...
        const fullDevDisposable = vscode.commands.registerCommand('codeagent.fullDevelopment', async (query) => {
        ...
        const developProjectDisposable = vscode.commands.registerCommand('codeagent.developProject', async (query) => {
        ...
        const panel = vscode.window.createWebviewPanel('codeAgentPersistent', 'Code Agent', vscode.ViewColumn.One, {
            enableScripts: true,
